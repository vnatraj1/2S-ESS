...
!  New for Version 2.4, surface-leaving Jacobians
...
!  Linearized (LPS)
      ...
      CALL TWOSTREAM_LPS_CHECK_INPUT_DIMS &
...
!  ---------------------
      ...
      IF (DO_DEBUG_INPUT) THEN
        ...
        IF (DO_PROFILE_WFS .OR. DO_SURFACE_WFS) &
          CALL TWOSTREAM_DEBUG_LPS_INPUT_MASTER()
      END IF
...
! @@ Rob Spurr, 05 November 2013, Version 2.3 --> BOA_TOA Flux outputs
      ...
      PROFJACFLUXES_TOA (:,:,:,:) = zero
      PROFJACFLUXES_BOA (:,:,:,:) = zero
...
!  Local linearization flags and numbers
      ...
      IF ( do_PROFILE_WFS ) THEN
         VFLAG  (1:NLAYERS) = LAYER_VARY_FLAG  (1:NLAYERS)
         VNUMBER(1:NLAYERS) = LAYER_VARY_NUMBER(1:NLAYERS)
      ENDIF
      ...
      IF ( DO_PROFILE_WFS ) THEN
         CALL TWOSTREAM_LP_QSPREP &
           ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,& ! Dimensions
             ...
             DO_PLANE_PARALLEL, VFLAG, VNUMBER,                  & ! Inputs
         ...
         IF ( DO_PROFILE_WFS  ) THEN
            CALL TWOSTREAM_THERMALSETUP_PLUS &
              ( MAXLAYERS, MAX_ATMOSWFS, NLAYERS,           & ! Input Numbers
                DO_PROFILE_WFS, VFLAG, VNUMBER,             & ! Input Linearization control
...
!  !@@ Call statement changed  to include TCutoff                   !@@ 2p4  05/14/15
         ...
         CALL TWOSTREAM_LPS_FOURIER_MASTER &
           ( MAXLAYERS, MAXTOTAL, MAXBEAMS, MAX_USER_STREAMS,                      & ! Dimensions
             ...
             DO_PLANE_PARALLEL, DO_2S_LEVELOUT, DO_PROFILE_WFS, DO_SURFACE_WFS,    & ! Input flags !@@ 2p2
             ...
             VFLAG, VNUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,                          & ! Input L_control !@@ 2p3
             ...
             LP_INITIAL_TRANS, LP_AVERAGE_SECANT, LP_T_DELT_MUBAR,                 & ! In/Out L_Miscsetups
             L_T_DELT_USERM, LP_EMULT_UP, LP_EMULT_DN,                             & ! In/Out L_Miscsetups
...
!  Exception handling
         ...
         IF ( STATUS_SUB .NE. 0 ) THEN
            STATUS_EXECUTION = 1
            WRITE(CF,'(I2)')FOURIER
            E_TRACE_2 = 'Error from 2S_LPS_FOURIER_MASTER, Fourier # '//CF
            RETURN
         ENDIF
                  ...
                  IF ( DO_PROFILE_WFS ) THEN
                     CALL TWOSTREAM_LPS_CONVERGE_OBSGEO  &
                       ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAX_GEOMETRIES,                & ! Dimensions
                         ...
                         DO_2S_LEVELOUT, DO_M1_SURFACE, DO_TSURFACE_WFS,                              & ! Inputs     ! @@ 2p2
                         NLAYERS, IBEAM, FOURIER, VFLAG, VNUMBER, N_TSURFACE_WFS, AZMFAC,             & ! Inputs
                  ...
                  IF ( DO_PROFILE_WFS ) THEN
                     CALL TWOSTREAM_LPS_CONVERGE &
                       ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAX_GEOMETRIES, MAXLAYERS,  & ! Dimensions
                         ...
                         VFLAG, VNUMBER, N_TSURFACE_WFS,  AZMFAC, UMOFF,                           & ! Inputs
                         PROFILEWF_F_UP,  PROFILEWF_F_DN, PROFJACLEVEL_F_UP, PROFJACLEVEL_F_DN,    & ! Inputs     ! @@ 2p2
                         SURFACEWF_F_UP,  SURFACEWF_F_DN, SURFJACLEVEL_F_UP, SURFJACLEVEL_F_DN,    & ! Inputs     ! @@ 2p2
                         PROFILEWF_TOA,   PROFILEWF_BOA,  PROFJACLEVEL_UP,   PROFJACLEVEL_DN,      & ! In/Out     ! @@ 2p2
                         SURFACEWF_TOA,   SURFACEWF_BOA,  SURFJACLEVEL_UP,   SURFJACLEVEL_DN  )      ! In/Out     ! @@ 2p2
...
!  Finish
      ...
      SUBROUTINE TWOSTREAM_DEBUG_LPS_INPUT_MASTER()

      CALL TWOSTREAM_WRITE_LPS_INPUT ( &
        DO_PROFILE_WFS, DO_SURFACE_WFS, DO_SLEAVE_WFS, &
      ...
      END SUBROUTINE TWOSTREAM_DEBUG_LPS_INPUT_MASTER
...
!  Simulation only flag
      ...
      DO_SIM_ONLY = (.not.DO_PROFILE_WFS .and..not.DO_SURFACE_WFS .and..not. DO_SLEAVE_WFS )
...
!     Version 2p4. Green's function linearization, 07 January 2015
               ...
               CALL TWOSTREAM_BVP_LP_SOLUTION_MASTER &
                 ( MAXLAYERS, MAXTOTAL, MAXBEAMS, MAX_ATMOSWFS,                    & ! Dimensions
                   ...
                   DO_PLANE_PARALLEL, FOURIER, IBEAM, NLAYERS, NTOTAL, NV, NVARY,  & ! inputs, Numbers
               ...
               IF ( DO_UPWELLING .and. DO_POSTPROCESSING ) THEN
                  CALL TWOSTREAM_UPUSER_PROFILEWF & 
                    ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,             & ! Dimensions
                      ...
                      N_USER_STREAMS, NV, NVARY, PI4, DELTAU_VERT, USER_STREAMS,       & ! input
               ...
               IF ( DO_DNWELLING .and. DO_POSTPROCESSING) THEN
                  CALL TWOSTREAM_DNUSER_PROFILEWF &
                    ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS, DO_USER_OBSGEOMS,   & ! Dimensions
                      ...
                      FOURIER, IBEAM, NLAYERS, TAYLOR_ORDER, TAYLOR_SMALL,                     & ! input
                      N_USER_STREAMS, NV, NVARY, PI4, DELTAU_VERT, USER_STREAMS,               & ! input
                      FLUXMULT, LAYER_PIS_CUTOFF, INITIAL_TRANS, AVERAGE_SECANT, T_DELT_MUBAR, & ! Input
               ...
               IF ( DO_INCLUDE_MVOUT ) THEN
                  CALL TWOSTREAM_FLUXES_PROFILEWF &
                    ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS,            & ! Dimensions
                      ...
                      NV, NVARY, PI4, STREAM_VALUE, FLUXMULT,       & ! Input Control
...
!         !@@ 2p2 New 2S_LEVELOUT option 07/17/13
!         !@@ 2p3. 11/5/13. Post-processing control
!         !@@ 2p4. 1/7/15 . Greens function solutions
