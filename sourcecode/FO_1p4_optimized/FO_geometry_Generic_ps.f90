...
module FO_geometry_Generic_PS_m
...
subroutine FindSun(DoNadir,Do_OverheadSun,Radius,SolarDirection,CumAngle,theta_boa_R,theta,stheta,DirSun)
...
!  Outputs
   ...
   real(ffp) , Intent(out)   :: theta,stheta
   logical   , Intent(Out)   :: DirSun

!  Local

   real(ffp) :: px(3),b,ctheta
...
!  Output
   ...
   real(ffp), Intent(Out)   :: Sunpaths(maxlayers)

!  Local

   integer    :: n1, k
   real(ffp)  :: sth0, th0, sth1, th1, ks1, radiik, radiik1
   real(ffp), parameter :: zero = 0.0_ffp
   real(ffp), parameter :: one  = 1.0_ffp
...
!  SBOA condition
   ...
   if ( Do_ZeroSunBOA ) then
      radiik = radii(n1)
      sunpaths(n) = radiik - Radstart
      do k = n1, 1, -1
         radiik1 = radii(k-1)
         sunpaths(k) = radiik1 - radiik
         radiik = radiik1
      enddo
      return
   endif
...
!  Local
   ...
   logical    :: trawl
   integer    :: n1, k
   real(ffp)  :: sth0, th0, sth1, th1, ks1, tanr, radiik, radiik1
...
!  Distances for layers N and below to NT
   ...
   if ( nt .gt. n ) then
      ...
      do k = n+1,nt-1
        ...
        th1  = asin(sth1) ; ks1  = th0-th1
        sunpaths(k) = two*sin(ks1)*radiik/sth0
        sth0 = sth1 ; th0 = th1
        radiik1 = radiik
      enddo
      sth1 = one ; ks1 = 0.5_ffp * pie - th0
      sunpaths(nt) = two*sin(ks1)*radiik1
...
!  Trawl up from layers above n, to TOA
   ...
   do k = n1, 1, -1
      ...
      th1  = asin(sth1)
      ks1  = th0-th1
      sunpaths(k) = sin(ks1)*radiik/sth1 
      sth0 = sth1
      th0  = th1
      radiik = radiik1
   enddo
...
end module FO_geometry_Generic_PS_m
