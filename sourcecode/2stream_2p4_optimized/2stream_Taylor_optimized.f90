...
!  other terms; evaluate multiplier

   mult = dm1 ; power = 1.0_dp
   do m = 2, mterms
      dm1 = delta * dm1 / real(m,dp)
      power = power * eps ; mult = mult + power * dm1      
   enddo
   mult = mult * udel * sm
...
!  local declarations
   ...
   REAL(kind=dp) :: dm, dm1, dm2

!  exp(De) expansion coefficients

   mterms = order + 2 ; dm1 = 1.0_dp ; d(0) = dm1
   do m = 1, mterms
      dm1  = delta * dm1 / real(m,dp)
      d(m) = dm1
   enddo
...
!  Series 3 absent for HMULT/EMULT, only present for GMULT
   ...
   power = 1.0_dp
   dm2 = d(2) ; dm1 = d(1) ; dm = d(0)
   series1 = dm - dm1*sm
   series2 = dm2 - dm1*delta
   series3 = dm2
   do m = 1, order
      dm = dm1 ; dm1 = dm2 ; dm2 = d(m+2)
      power = power * eps
      series1 = series1 + power * (dm  - dm1*sm)
      series2 = series2 + power * (dm2 - dm1*delta)
      series3 = series3 + power * dm2
   enddo
...
!  Local
   ...
   REAL(kind=dp)  :: dm1, acm1, bcm1, dmj, ccm1, Term_1am
...
!  Develop ( 1 - x/s )^-2  for small x, need Order+2 terms
   ...
   y1 = 1.0_dp / y
   acm1  = 1.0_dp ; bcm1  = 1.0_dp
   ac(0) = acm1 ; bc(0) = bcm1
   do m = 1, mterms2
      acm1  = y1 * acm1
      ac(m) = acm1
      bcm1  = y1 * bcm1 * real(m+1,dp) / real(m,dp)
      bc(m) = bcm1
   enddo
...
!  Term 1a: Use CC
   ...
   ccm1 = cc(0)
   Term_1a(0) = CONS1 * ccm1
   do m = 1, mterms1
      Term_1am = CONS2 * ccm1
      ccm1 = cc(m)
      Term_1am = Term_1am + CONS1 * ccm1
      Term_1a(m) = Term_1am
   enddo
...
!  Term 1a: Use AC and CC
   ...
   ccm1 = cc(0)
   Term_1a(0) = CONS1 * ccm1 + CONS3 * ac(0)
   do m = 1, mterms1
      Term_1am   = CONS2 * ccm1 + CONS3 * ac(m)
      ccm1 = cc(m)
      Term_1am   = Term_1am + CONS1 * ccm1
      Term_1a(m) = Term_1am
   enddo
