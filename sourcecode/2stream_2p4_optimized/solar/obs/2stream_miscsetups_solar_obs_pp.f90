...
! #                                                             #
! #           TWOSTREAM_QSPREP_PREPTRANS_EMULT_OBS_PP           #
! #                                                             #
! ###############################################################

module twostream_miscsetups_solar_obs_pp_m
...
SUBROUTINE TWOSTREAM_QSPREP_PREPTRANS_EMULT_OBS_PP &
...
!  Average secants
      ...
      REAL(kind=dp), INTENT(IN)  :: AVERAGE_SECANT_PP ( MAXGEOMS )
...
!  Derived optical thickness inputs
      ...
      REAL(kind=dp) :: SB, USIB, TAUGRID, TAUGRIDN1, TAUSLANT, DELTAU_VERTN
...
!  Initial transmittance and transmittance factors for average secant streams
!  ==========================================================================

        DELTAU_VERTN = DELTAU_VERT(N)
        TAUGRIDN1 = TAUGRID
        TAUGRID = TAUGRID + DELTAU_VERTN
        TAUSLANT = TAUGRID * SB
        IF ( N .LE. LAYER_PIS_CUTOFFB ) THEN
          IF ( TAUSLANT .GT. MAX_TAU_PATH ) THEN
            LAYER_PIS_CUTOFFB = N
          ENDIF
          INITIAL_TRANS_NIB    = EXP ( - TAUGRIDN1 * SB )
...
!  debug
!      do N = 1, nlayers
!      write(*,'(i3,1p2e20.10)')n,initial_trans(n,1),average_secant_pp(1)
!      enddo
...
!  finish
      ...
      RETURN
END SUBROUTINE TWOSTREAM_QSPREP_PREPTRANS_EMULT_OBS_PP

end module twostream_miscsetups_solar_obs_pp_m
