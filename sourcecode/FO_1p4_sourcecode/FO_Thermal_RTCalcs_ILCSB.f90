...
module FO_Thermal_RTCalcs_ILCSB_m

!  For a given wavelength, this routine will calculate upwelling and downwelling
!  First Order Intensities(I), and any number of LCS Jacobians (column/surface) and Blackbody Jacobians
...
!       DTE_Integral_ILCSB_UP   (Upwelling only)
!       DTE_Integral_ILCSB_DN   (Downwelling only)
!       DTE_Integral_ILCSB_UPDN (Upwelling and Downwelling)
...
subroutine DTE_Integral_ILCSB_UP &
   ( maxgeoms, maxlayers, maxfinelayers, max_user_levels,         & ! Inputs (dimensioning)
     ...
     LC_Jacobians_dta_up, LC_Jacobians_dts_up, LS_Jacobians_dts,  & ! Outputs (LCS Jacobian)
...
!    computation of Radiances and LCS Jacobians and blackbody Jacobians. 

!  Inputs: geometry, optical properties, Planck functions, emissivity

!  Original LCS version, revised by R. Spurr, 01 June 2012
!   Extension to multiple geometries, 19 December 2012

!  This LCSB version, the first of its kind, programmed by R. Spurr 4/8/19.
!    - Based on the DTE_Integral_ILCS_UP subroutine from Version 1.4 of the FO CODE.
...
!  Jacobians
   ...
   real(fpk), Intent(Out)  :: LC_Jacobians_dta_up  ( max_user_levels, maxgeoms, max_atmoswfs )
   real(fpk), Intent(Out)  :: LC_Jacobians_dts_up  ( max_user_levels, maxgeoms, max_atmoswfs )
...
!  Help
   ...
   integer    :: n, j, nj, q, jb, v, uta, nstart, nc, nut, nut_prev, Qnums(maxlayers)
...
!  Column Wfs (Atmospheric term)
      ...
      if ( do_columnwfs ) then
         ...
         DO UTA = N_USER_LEVELS, 1, -1
            ...
            ENDDO
            do q = 1, n_columnwfs
               LC_Jacobians_dta_up(UTA,V,Q) = L_CUMSOURCE(Q)
            enddo
            IF ( NUT .NE. NUT_PREV ) NSTART = NUT - 1
            NUT_PREV = NUT
         ENDDO
...
!  Column Wfs (surface emission term)
      ...
      if ( do_columnwfs ) then
         ...
         DO UTA = N_USER_LEVELS, 1, -1
            ...
            ENDDO
            do q = 1, n_columnwfs
               LC_Jacobians_dts_up(UTA,V,Q) = L_CUMSOURCE(Q)
            enddo
            IF ( NUT .NE. NUT_PREV ) NSTART = NUT - 1
            NUT_PREV = NUT
         ENDDO
...
!  Finish
   ...
   return
end subroutine DTE_Integral_ILCSB_UP

!

subroutine DTE_Integral_ILCSB_DN &
   ( maxgeoms, maxlayers, maxfinelayers, max_user_levels,         & ! Inputs (dimensioning)
     ...
     intensity_dta_dn, LC_Jacobians_dta_dn, LAB_Jacobians_dta_dn, & ! Outputs (Main)
...
!  This LCSB version, the first of its kind, programmed by R. Spurr 4/8/19.
!   Based on the DTE_Integral_ILCS_DN subroutine from Version 1.4 of the FO CODE.
...
!  Jacobians
   ...
   real(fpk), Intent(Out)  :: LC_Jacobians_dta_dn   ( max_user_levels, maxgeoms, max_atmoswfs )
...
!  Help
   ...
   integer    :: n, j, nj, q, v, jb, uta, nstart, nc, nut, nut_prev, Qnums(maxlayers)
...
!  Column Wfs (Atmospheric term)
      ...
      if ( do_columnwfs ) then
         ...
         DO UTA = 1, N_USER_LEVELS
            ...
            ENDDO
            do q = 1, n_columnwfs
               LC_Jacobians_dta_dn(UTA,V,Q) = L_CUMSOURCE(Q)
            enddo
            IF ( NUT .NE. NUT_PREV ) NSTART = NUT + 1
            NUT_PREV = NUT
         ENDDO
...
!  Finish
   ...
   return
end subroutine DTE_Integral_ILCSB_DN

!

subroutine DTE_Integral_ILCSB_UPDN &
   ( maxgeoms, maxlayers, maxfinelayers, max_user_levels,         & ! Inputs (dimensioning)
     max_atmoswfs, max_surfacewfs, do_abbwf, do_sbbwf,            & ! Inputs (dimensioning, BB flags)
     ...
     LC_Jacobians_dta_up, LC_Jacobians_dts_up, LAB_Jacobians_dta_up, & ! Outputs (Up Jacobians)
     LS_Jacobians_dts,    LSB_Jacobians_dts,                         & ! Outputs (Surf/SBB Jacobians)
     LC_Jacobians_dta_dn, LAB_Jacobians_dta_dn,                      & ! Outputs (Down Jacobians)
     Tcom, L_tcom, LB_Tcom1, LB_TCom2 )                                ! Outputs (auxiliary)
...
!   Based on the DTE_Integral_ILCS_UPDN subroutine from Version 1.4 of the FO CODE.
...
!  Downwelling
   ...
   ( maxgeoms, maxlayers, maxfinelayers, max_user_levels,         & ! Inputs (dimensioning)
     ...
     Mu1, NCrit, Radcrit, CotCrit, Raycon, cota,                  & ! Inputs (Geometry)
     radii, xfine, wfine, csqfine, cotfine,                       & ! Inputs (Geometry)
     intensity_dta_dn, LC_Jacobians_dta_dn, LAB_Jacobians_dta_dn, & ! Outputs (Radiances,LC/LAB Jacobians)
     Tcom, L_Tcom, LB_Tcom1, LB_Tcom2 )                             ! Outputs (Auxiliary)
   endif
...
!  Finish
   ...
   return
end subroutine DTE_Integral_ILCSB_UPDN

!  End module

end module FO_Thermal_RTCalcs_ILCSB_m
